...
class MegaPiControllerNode(Node):
  ...
  def __init__(self, verbose=True, debug=False):
    ...
    self.debug = debug
    self.state = "run"
    self.create_subscription(Joy, '/joy', self.joy_callback, qos_profile_system_default)
  ...
  def joy_callback(self, joy_cmd: Joy):
    ...
    if self.verbose:
      print('state: ' + self.state + ' v_straight: ' + repr(int(round(v_straight, 2))) + '/' +
            repr(self.v_max_straight) + ' v_slide: ' + repr(int(round(v_slide, 2))) + '/' + repr(self.v_max_slide) +
