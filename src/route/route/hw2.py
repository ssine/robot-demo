...
# x, y, z, y, p, r
tag_positions = {0: [1.0, 0.0, 0.11, math.pi, -math.pi / 2, 0]}
...
class RoutePlanner(Node):
  ...
  def __init__(self, waypoints):
    ...
    self.pid = PIDcontroller(0.02, 0.005, 0.005)
  ...
  def tick(self):
    ...
    self.pub_twist.publish(genTwistMsg(coord(update_value, self.position)))
    self.position += update_value

  def on_detection(self, tags_msg: AprilTagDetectionArray):
    estimated_positions = np.empty((len(tags_msg.detections), 3))
    for idx, detection in enumerate(tags_msg.detections):
      est_pose = self.transform_pose(inv_pose(detection.pose), f'tag_{detection.id}', 'bot')
      ...
      estimated_positions[idx] = np.array([x, y, w])
